...
### EffectSystem（特效系统）
- **核心工具** → `Src/ECS/System/EffectSystem/EffectTool.cs`（统一入口 `EffectTool.Spawn`，Host? 可选参数区分独立/附着；不走 `EntityManager.Spawn`，而是内部执行 Effect 专用 Spawn 编排）
- **特效实体** → `Src/ECS/Entity/Effect/EffectEntity.cs`（Node2D + IEntity + IPoolable）
- **特效组件** → `Src/ECS/Component/Effect/EffectComponent/EffectComponent.cs`（完整特效管理：附着跟随、MaxLifeTime 计时器、通过 `GameEventType.Unit.PlayAnimationRequested` 事件驱动动画、宿主销毁监听）
...
- **特效 DataKey** → `Data/DataKeyRegister/Effect/DataKey_Effect.cs`
- **设计提示词** → `Docs/框架/ECS/Entity生成提示词/特效系统_Entity生成提示词.md`
