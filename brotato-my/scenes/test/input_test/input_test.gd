...
func update_vibration_status(weak: float, strong: float, duration: float, desc: String = "") -> void:
	if vibration_label: # 确保震动标签存在
		if duration > 0: # 正在震动中
			# 更新震动状态文本
			vibration_label.text = "震动状态: %s\n低频强度: %.2f\n高频强度: %.2f\n剩余时间: %.2f s" % [desc, weak, strong, duration]
			# 设置震动中颜色为红色
			vibration_label.add_theme_color_override("font_color", Color(1, 0.2, 0.2))
		else: # 震动已停止
			# 更新为待机状态文本
			vibration_label.text = "震动状态: 无\n低频强度: 0.00\n高频强度: 0.00\n等待触发..."
			# 设置待机颜色为橙色
			vibration_label.add_theme_color_override("font_color", Color(1, 0.5, 0))
