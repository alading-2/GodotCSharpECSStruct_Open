...
{
	// Pools are managed by ObjectPoolManager, we just hold references to prevent GC if needed, 
	// or just rely on the Manager.
	// For this test, we create them and let them register themselves.
	...
	private Label _globalStatsLabel;
	...
	{
		// 1. 初始化容器
		_gameContainer = new Node2D { Name = "GameContainer", ZIndex = 0 };
	...
	private void InitializePools()
	{
		...
		_projectilePool = new ObjectPool<TestProjectile>(
			...
			config: new ObjectPoolConfig { Name = "ProjectilePool", InitialSize = 20, MaxSize = 200 }
		);
		...
		_effectPool = new ObjectPool<TestEffect>(
			...
			config: new ObjectPoolConfig { Name = "EffectPool", InitialSize = 10, MaxSize = 50 }
		);
	}
	...
	{
		var p = _projectilePool.Spawn(_gameContainer);
	...
	{
		var e = _effectPool.Spawn(_gameContainer);
		var position = pos ?? GetRandomPos();
		e.Init(position);
	}
	...
	{
		// 清除旧的 Label (简单粗暴，生产环境建议用对象池管理 Label 或固定 Label 引用)
