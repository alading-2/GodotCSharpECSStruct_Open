...
AnimatedSprite2D "豺狼人"
  └─ CollisionShape2D        ← Collision Profile 实例（CharacterBody2D/Area2D，重命名为此名）
...
- **Collision Profile 模板**（`.tscn`）：仅设置 `collision_layer` / `collision_mask`，不内嵌形状。形状由生成器在每次生成时注入，各角色场景可独立调整。
