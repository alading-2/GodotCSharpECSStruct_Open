...
1. 插件文件应位于 `addons/SpriteFramesGenerator_GDS` 目录下。
2. 打开 Godot 编辑器。
3. 进入菜单栏 **Project (项目) -> Project Settings (项目设置) -> Plugins (插件)**。
4. 找到 **SpriteFrames Generator (GDS)** 并勾选 **Enable (启用)**。
...
| `UNIFIED_NAME_PATHS` | Dictionary | 统一动画命名规则，Key 为目标动画名，Value 为路径列表（Array） |
...
### A. Spine 导出格式
- **格式**：`前缀-动画名_序号.png`
- **示例**：`hero_guangfa-Attack1_00.png` → 动画名 `attack1`，第 0 帧

### B. 简单格式
- **格式**：`动画名_序号.png`
- **示例**：`Run_00.png` → 动画名 `run`，第 0 帧
