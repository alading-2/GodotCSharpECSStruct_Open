...
- **碰撞模板目录** → `Src/ECS/Component/Presets/Collision/`
- **碰撞模板说明** → `Src/ECS/Component/Presets/Collision/README.md`
- **SpriteFramesGenerator 规则** → `addons/SpriteFramesGenerator/sprite_frames_config.gd`
- **CollisionTypeRegistry 生成器** → `Tools/ResourceGenerator/ResourceGenerator.cs`
- **关键约定**：生成器把碰撞模板挂到 `VisualRoot/CollisionShape2D`；运行时由 `CollisionComponent` 统一扫描 `VisualRoot` 与实体 `Collision` 容器
