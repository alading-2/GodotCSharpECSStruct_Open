# 通用数学工具重构执行方案

> 状态：已完成第一轮落地
> 更新时间：2026-03-30

## 已完成

### 1. 曲线数学层

已新增：

- `Src/Tools/Math/Curves/ArcLengthLut.cs`
- `Src/Tools/Math/Curves/EllipseArc2D.cs`
- `Src/Tools/Math/Curves/Parabola2D.cs`
- `Src/Tools/Math/Curves/CircularArc2D.cs`

已形成统一能力：

- 曲线点采样
- 切线采样
- 弧长近似
- `progress -> t` 弧长映射

### 2. Boomerang 业务层收敛

`BoomerangStrategy` 已改为：

- 只保留去程 / 停顿 / 返程阶段机
- 曲线采样改为调用 `EllipseArc2D`
- 保留返程跟随宿主、返程倍率、回旋方向和停顿语义

### 3. 几何层通用化

已新增：

- `Src/Tools/Math/Geometry/Geometry2D.cs`

已完成兼容迁移：

- `GeometryCalculator` 保留为兼容门面
- `SpawnPositionCalculator` 已直接转调 `Geometry2D`
- `TargetSelector` 旧调用不需要一次性重写

### 4. 运动模式扩展

已新增：

- `MoveMode.Parabola`
- `MoveMode.CircularArc`
- `ParabolaStrategy`
- `CircularArcStrategy`

已新增参数：

- `MovementParams.ParabolaApexHeight`
- `MovementParams.CircularArcRadius`
- `MovementParams.CircularArcClockwise`

## 本轮验证

已完成：

- `dotnet build Brotato_my.sln`
- 新增数学测试：`Src/Test/SingleTest/Tools/Math/MyMathTest.cs`
- 扩展运动场景测试：`Src/Test/SingleTest/ECS/Movement/MovementComponentTestScene.cs`

说明：

- 当前仓库仍有大量既有 XML 注释警告和空引用警告，不属于本轮引入
- 本轮编译结果为 0 error

## 当前边界

- 运行时数学类型继续使用 Godot `Vector2`
- 不引入新的运行时第三方数学库
- `TargetSelectorQuery`、`GeometryType` 仍属于领域层，不下沉到数学层
- `GeometryCalculator` 暂不删除，继续作为过渡兼容层

## 暂缓项

以下内容不进入本轮：

- Godot `Curve2D / Path2D / Tween` 运行时接入
- 更大的曲线接口抽象体系
- 全量迁移到 `System.Numerics`
- 更多样条曲线类型（如 Catmull-Rom、Hermite）
