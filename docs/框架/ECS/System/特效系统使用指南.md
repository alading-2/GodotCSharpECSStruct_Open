...
EffectTool.Spawn(position, new EffectSpawnOptions(
    VisualScene: scene,
    Name: "爆炸特效"
));
```
...
public AbilityExecutedResult Execute(CastContext context)
{
    ...
    foreach (var target in context.Targets ?? [])
    {
        ...
        {
            EffectTool.Spawn(targetNode.GlobalPosition, new EffectSpawnOptions(
                VisualScene: effectScene,
                Name: "命中特效"
            ));
        }
...
- **工具入口** → `Src/ECS/System/EffectSystem/EffectTool.cs`
- **特效组件** → `Src/ECS/Component/Effect/EffectComponent/EffectComponent.cs`
- **特效实体** → `Src/ECS/Entity/Effect/EffectEntity.cs`
- **特效数据键** → `Data/DataKeyRegister/Effect/DataKey_Effect.cs`
