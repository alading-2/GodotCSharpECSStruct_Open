...
### 标准的 Godot CSharp 脚本模板

```C#
using Godot;
// 1. 你的最爱：静态导入，像写 GDS 一样写代码
using static Godot.GD;

// 2. 必须是 partial 类，且类名必须和文件名一致
public partial class MyPlayer : Sprite2D { // 假设你挂载在一个 Sprite2D 节点上

    // 3. [Export] 让变量显示在 Godot 编辑器的属性面板里，方便策划/美术调整数值
    [Export]
    public float Speed { get; set; } = 400.0f;

    // --- 生命周期：第一阶段 ---
    // _Ready: 当节点进入场景树时调用（相当于 Unity 的 Start 或 构造函数逻辑）
    // 用途：获取子节点、初始化变量、绑定信号
    public override void _Ready() {
        Print("游戏开始，节点已加载！");

        // 获取子节点（泛型不仅有提示，还不用强转）
        // 相当于 TS 里的 this.querySelector<Label>("Name")
        var myLabel = GetNode<Label>("UI/ScoreLabel");
        myLabel.Text = "Ready";
    }

    // --- 生命周期：第二阶段 ---
    // _Process: 每一帧都会运行（相当于 Update）
    // delta: 上一帧到这一帧的时间间隔（秒），用于保证移动速度与帧率无关
    public override void _Process(double delta) {

        // 4. 处理输入
        if (Input.IsActionPressed("ui_right")) {
            // 5. 修改自身属性 (Position, Rotation, Scale 是内置属性)
            // 因为继承了 Sprite2D，直接用 Position
            Position += new Vector2(Speed * (float)delta, 0);
        }

        // 旋转自己
        Rotation += 1.5f * (float)delta;
    }
}
```
