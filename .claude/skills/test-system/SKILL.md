...
- 维护 `SelectedEntity`
- 通过 `TestSelectionContext.Events` 广播选中实体变化
- 处理模块切换与刷新

不要把具体业务测试逻辑继续堆进 `TestSystem.cs`。

`TestSystem` 和 `TestSelectionContext` 的共享状态广播统一走 `EventBus`，不要新增 C# `event` 给宿主和模块做联动。
...
在 `TestSystem.tscn` 的 `ModuleHost` 下挂载 `MyTestModule.tscn`，由 `TestSystem` 在 `_Ready()` 扫描 `TestModuleBase` 子节点完成注册。
