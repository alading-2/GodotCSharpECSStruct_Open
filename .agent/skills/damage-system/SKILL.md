...
DamageService.Instance.Process(new DamageInfo
{
    ...
    Instigator = playerEntity,    // 真正施法者（统计归属）
    Victim = enemyEntity,
    BaseDamage = 50f,
...
DamageService.Instance.Process(new DamageInfo
{
    ...
    Victim = target,
    BaseDamage = context.Caster.Data.Get<float>(DataKey.AttackDamage) * 1.5f,
...
| 优先级 | 处理器 | 职责 |
|--------|--------|------|
| 100 | BaseDamageProcessor | 基础检查（死亡/无敌/基础数值） |
...
{
    public IEntity? Attacker;       // 直接来源
    public IEntity? Instigator;     // 施法者（统计用）
    public IEntity? Victim;         // 受害者
    public float BaseDamage;        // 基础伤害
    public DamageType DamageType;   // Physical / Magic / True / ...

    // 管道中间状态（处理器读写）
    public float Amount;            // 当前计算中的伤害值
