...
public partial class AttributeComponent : Node
{
    ...
    // ================= Computed Properties =================
    // 业务代码应始终访问这些属性，而不是 Data 中的 Base 属性。
    ...
    /// 获取实体的最终暴击率。
    /// </summary>
    public float CritRate => GetFinalValue("CritRate", "BaseCritRate");

    /// <summary>
    /// 获取实体的最终暴击倍率。
    /// </summary>
    public float CritMultiplier => GetFinalValue("CritMultiplier", "BaseCritMultiplier");
    ...
    public override void _Ready()
    {
        ...
        Log.Debug($"属性组件初始化完成: 攻击力={Damage}, 速度={Speed}");
    }
    ...
    public float GetFinalValue(string attrName, string baseAttrKey, float defaultBaseValue = 0f)
    {
    ...
    private void RecalculateAll()
    {
        ...
        // 预计算常用的核心属性，提高访问速度
        _cachedValues["Damage"] = CalculateFinalValue("Damage", _data.Get<float>("BaseDamage"));
        _cachedValues["Speed"] = CalculateFinalValue("Speed", _data.Get<float>("BaseSpeed"));
        _cachedValues["CritRate"] = CalculateFinalValue("CritRate", _data.Get<float>("BaseCritRate"));
        _cachedValues["CritMultiplier"] = CalculateFinalValue("CritMultiplier", _data.Get<float>("BaseCritMultiplier"));
