/// <summary>
/// 碰撞类型查询工具 - 提供 CollisionType 与 (layer, mask) 的正向/反向查询方法
/// <para>
...
public static class CollisionTypeQuery
{
    ...
    /// <param name="type">查找成功时输出对应的 CollisionType，失败时输出 None</param>
    /// <returns>true = 找到完全匹配的注册类型；false = 未在注册表中找到（None 或未知配置）</returns>
    ...
    /// 仅支持单场景位标志（如 EffectCollision），不支持组合类型（如 VisualBody）。
    /// </summary>
    /// <param name="type">目标碰撞类型（单场景位标志）</param>
    /// <param name="layer">输出 collision_layer</param>
    /// <param name="mask">输出 collision_mask</param>
    /// <returns>true = 找到注册数据；false = 组合类型或未注册（输出均为 0）</returns>
