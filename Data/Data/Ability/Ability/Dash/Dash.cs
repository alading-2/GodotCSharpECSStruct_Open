...
/// 效果：向施法者当前移动方向（或面朝方向）瞬间位移 AbilityRange 距离
/// </summary>
public class DashExecutor : IAbilityExecutor
{
    ...
    public AbilityExecutedResult Execute(CastContext context)
    {
        ...
        // 2. 确定冲刺方向：优先使用当前移动方向，否则用 FlipH 判断面朝方向
        ...
        // 4. 生成附着特效（跟随施法者，播完自动销毁）
        var effectScene = ResourceManagement.Load<PackedScene>(ResourcePaths.Asset_Effect_004龙卷风, ResourceCategory.Asset);
        if (effectScene != null)
        {
