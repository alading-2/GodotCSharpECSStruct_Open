...
`TestSystem` 自身只负责宿主、模块切换和选中实体，不要让模块去赌启动时序。
如果模块依赖 `DataRegistry` / `ResourcePaths` / 配置目录，必须满足两点：

- 启动时显式准备必要元数据，或在首次使用前显式触发初始化
- 空结果不能静默缓存，必须支持后续重试，并在仍为空时 `Log.Error`
...
- 在 `TestSystem.tscn` 的 `ModuleHost` 下挂载 `MyTestModule.tscn`
- `TestSystem` 会在 `_Ready()` 扫描 `ModuleHost` 下的 `TestModuleBase` 子节点并自动注册
- 如果模块依赖 `DataKey` 分类元数据，确保在模块初始化前完成静态注册或在模块内部实现可重试加载
