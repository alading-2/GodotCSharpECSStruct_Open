...
- 维护 `SelectedEntity`
- 处理模块切换与刷新
- 通过全局事件发起 / 取消鼠标选择请求，并消费 `PrimaryEntity / Entities` 选择结果
...
- 应拆成独立 AutoLoad 系统
- `TestSystem` 只发开始/取消事件，并监听选择完成或未命中事件
- 调试面板默认使用 `ClickOrDragBox`，单击用于选主目标，拖拽用于显示框选预览并返回实体集合
- 调试面板默认可以用宽松请求（任意实体类型 + 距离兜底），正式玩法的 RTS 选择应在请求中指定 `CollisionMask / EntityType / Team`
